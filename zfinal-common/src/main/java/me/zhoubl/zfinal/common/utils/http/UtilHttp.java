package me.zhoubl.zfinal.common.utils.http;
...
public class UtilHttp {
	...
	private static HttpsURLConnection initHttps(String url, String method, Map<String, String> headers)
		...
		// 设置域名校验
		http.setHostnameVerifier(new UtilHttp().new TrustAnyHostnameVerifier());
		// 连接超时
		http.setConnectTimeout(DEF_CONN_TIMEOUT);
		// 读取超时 --服务器响应比较慢，增大时间
		http.setReadTimeout(DEF_READ_TIMEOUT);
	...
	/**
	 * 
	 * @description 功能描述: get请求  发送byte流
	 * @return 返回类型:
	 * @throws Exception
	 */
	public static void getByByte(String url, Map<String, String> params, Map<String, String> headers,OutputStream out) throws Exception {
		HttpURLConnection http = null;
		if (isHttps(url)) {
			http = initHttps(initParams(url, params), _GET, headers);
		} else {
